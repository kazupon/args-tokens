 ...
 * present, and `false` makes a positional argument optional. `S` stays as is when `R` is not `true`
 * or `false`, such as when the options have no `required` or a `boolean` one.
 */
type WithRequiredOption<S, R> = [R] extends [boolean]
  ? boolean extends R
    ? S
