...
`parse` is called synchronously, and what it returns becomes the value, so throw to reject a value. An `async` function returns a promise, and the promise becomes the value as is: `resolveArgs()` and `parse()` do not await it, and do not report its rejection as a validation error. Await the value and handle the rejection yourself.
