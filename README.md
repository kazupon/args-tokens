...
A `-` inside a group, as in `-o-` or `-p-5`, does not end the options: the rest of the group from the `-` is the value of the option before it, in a value token with `inlineValue: false`, since no `=` is written. So `-o- input.txt` gives `-o` the value `-`, and `input.txt` is read as usual. `allowCompatible: true` keeps the `node:util` tokens, where the `-` becomes the option terminator.
