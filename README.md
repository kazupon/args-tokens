...
The default of an `enum` option with `choices` is checked when it would be used, that is, when the option is not given or its value is missing or rejected. A default that is not one of the choices is reported as `ArgsValidationErrorKeys.invalidDefault` and is not used. Its `values` has the same keys as that of `ArgsValidationErrorKeys.invalidChoice`, with the default as `actual`. With a `parse` function of your own, or a `map()` transform, the default is a value that the function returns, which need not be one of the choices, and it is not checked. `choice()` returns the value as is, so its default is checked.
