...
`next`, `suggestion` and the hint in the message are given only when the option is known to take that value: a `number` option gets them only for a numeric value, and an `enum` option with `choices` only for one of the choices, with or without a `parse` function. So `--port -x` gets no suggestion, and neither does `--level -x` for an `enum` option with `choices: ['debug', 'info']`. A built-in combinator, such as `integer()`, `float()`, `number({ min: 1 })` or `string({ pattern })`, is also checked with its own `parse` function, which has no side effects, so `--count -x` gets no suggestion for `integer()` and `--count -5` does. A `parse` function of your own is never called with a value that the option was not given, so an option with one gets the suggestion only when its type or `choices` allow the value, and a `string`, a `custom` or an `enum` without `choices` with such a function gets none.
